...
bool Object::load_simple_mtl(const std::string& filename)
{
  ...
  float Object::max_x(float trans, float scale){
    ...
    for (size_t i=0; i<m_groups.size(); ++i)
    {
      Group& group = m_groups[i];
       ...
       {
        if(i==0)
          max_vx = (group.m_vertices[i].x)*scale + trans;

        else{
          if((group.m_vertices[i].x)*scale + trans > max_vx)
            max_vx = (group.m_vertices[i].x)*scale + trans;
        } 
       }
  ...
  float Object::max_y(float trans, float scale){
    ...
    for (size_t i=0; i<m_groups.size(); ++i)
    {
      Group& group = m_groups[i];
       ...
       {
        if(i==0)
          max_vy = (group.m_vertices[i].y)*scale + trans;

        else{
          if((group.m_vertices[i].y)*scale + trans > max_vy)
            max_vy = (group.m_vertices[i].y)*scale + trans;
        } 
       }
  ...
  float Object::max_z(float trans, float scale){
    ...
    for (size_t i=0; i<m_groups.size(); ++i)
    {
      Group& group = m_groups[i];
       ...
       {
        if(i==0)
          max_vz = (group.m_vertices[i].z)*scale + trans;

        else{
          if((group.m_vertices[i].z)*scale + trans > max_vz)
            max_vz = (group.m_vertices[i].z)*scale + trans;
        } 
       }
  ...
  float Object::min_x(float trans, float scale){
    ...
    for (size_t i=0; i<m_groups.size(); ++i)
    {
      Group& group = m_groups[i];
       ...
       {
        if(i==0)
          min_vx = (group.m_vertices[i].x)*scale + trans;

        else{
          if((group.m_vertices[i].x)*scale + trans < min_vx)
            min_vx = (group.m_vertices[i].x)*scale + trans;
        } 
       }
  ...
  float Object::min_y(float trans, float scale){
    ...
    for (size_t i=0; i<m_groups.size(); ++i)
    {
      Group& group = m_groups[i];
       ...
       {
        if(i==0)
          min_vy = (group.m_vertices[i].y)*scale + trans;

        else{
          if((group.m_vertices[i].y)*scale + trans < min_vy)
            min_vy = (group.m_vertices[i].y)*scale + trans;
        } 
       }
  ...
  float Object::min_z(float trans, float scale){
    ...
    for (size_t i=0; i<m_groups.size(); ++i)
    {
      Group& group = m_groups[i];
       ...
       {
        if(i==0)
          min_vz = (group.m_vertices[i].z)*scale + trans;

        else{
          if((group.m_vertices[i].z)*scale + trans < min_vz)
            min_vz = (group.m_vertices[i].z)*scale + trans;
        } 
       }
  ...
  float Object::get_max_x(){
      return max_vx;
  }
  float Object::get_max_y(){
      return max_vy;
  }
  float Object::get_max_z(){
      return max_vz;
  }
  float Object::get_min_x(){
      return min_vx;
  }
  float Object::get_min_y(){
      return min_vy;
  }
  float Object::get_min_z(){
      return min_vz;
  }

  glm::vec3 Object::get_max_pos(){
      return glm::vec3(max_vx, max_vy, max_vz);
  }
  glm::vec3 Object::get_min_pos(){
      return glm::vec3(min_vx, min_vy, min_vz);
  }